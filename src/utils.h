...
template <int16_t N = 1000> class Palette {
...
private:
  void updatePalette() {
    for (int i = 1; i < SIZE; ++i) {
      []<std::size_t... I>(const auto &fn, const auto &tup,
      ...
      (
          [&](const auto &t, const size_t ind) {
            if (ind == mPaletteId)
              mPalette[i] = t(static_cast<double>(i) / SIZE);
            return 0;
          },
          paletteFns,
          std::make_index_sequence<std::tuple_size_v<decltype(paletteFns)>>());
    }
  }
